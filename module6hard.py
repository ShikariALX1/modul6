...
class Figure:
    ...
    def __is_valid_sides(self, *new_sides):
        return (len(new_sides) == self.sides_count and
                all(isinstance(x, int) and x > 0 for x in new_sides))
...
class Triangle(Figure):
    ...
    def get_square(self):
        ...
        return math.sqrt(s * (s - self.get_sides()[0]) *
                         (s - self.get_sides()[1]) * (s - self.get_sides()[2]))
...
class Cube(Figure):
    ...
    def __init__(self, color, cube_side):
        sides = [cube_side] * self.sides_count
